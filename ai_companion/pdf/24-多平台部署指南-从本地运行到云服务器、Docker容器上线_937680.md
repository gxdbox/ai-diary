...
<audio controls src="audio/24-多平台部署指南_937680_d6ec8db56d.mp3"></audio>
...
<img src="image/24-多平台部署指南_937680_ca14b27d3a.jpg" alt="" title="表 24-1 部署前准备的维度" loading="eager" decoding="sync" referrerpolicy="no-referrer"/>
...
<img src="image/24-多平台部署指南_937680_42d3b396a5.png" alt="图片" title="图 24-1 项目 docker 架构" loading="eager" decoding="sync" referrerpolicy="no-referrer"/>
...
<img src="image/24-多平台部署指南_937680_ec120b448f.jpg" alt="" title="表 24-2 访问的服务" loading="eager" decoding="sync" referrerpolicy="no-referrer"/>
...
<img src="image/24-多平台部署指南_937680_58a0f8581c.png" alt="图片" title="图 24-2 阿里云实例" loading="eager" decoding="sync" referrerpolicy="no-referrer"/>
...
<img src="image/24-多平台部署指南_937680_b6db4f4c74.png" alt="图片" title="图 24-3 安全组设置" loading="eager" decoding="sync" referrerpolicy="no-referrer"/>
...
部署方式对比如表24-3所示。
<img src="image/24-多平台部署指南_937680_12b15eba0d.jpg" alt="" title="表 24-3 部署方式对比" loading="eager" decoding="sync" referrerpolicy="no-referrer"/>
...
Grafana仪表板：访问 http://localhost:3001 登录Grafana（默认账号：admin/admin），查看预配置的仪表板。注意：如果Grafana使用默认3000端口，需要修改docker-compose.yml中的端口映射为3001:3000，以避免与前端服务端口冲突。
...
young.lv · 2026-01-16 10:54:44 · 点赞 0

Error response from daemon: failed to create task for container: failed to create shim task: OCI runtime create failed: runc create failed: unable to start container process: error during container init: error mounting "/opt/emotional_chat-main/emotional_chat-main/monitoring/prometheus.yml" to rootfs at "/etc/prometheus/prometheus.yml": mount /opt/emotional_chat-main/emotional_chat-main/monitoring/prometheus.yml:/etc/prometheus/prometheus.yml (via /proc/self/fd/6), flags: 0x5000: not a directory: unknown: Are you trying to mount a directory onto a file (or vice-versa)? Check if the specified host path exists and is the expected type执行了docker compose up -d 以后，生成了emotional_chat-main/monitoring/prometheus.yml，但是，prometheus.yml是一个文件夹，所以就报错了。是不是哪里配置有问题

讨论串（1）

讨论 #1

袁从德 · 2026-01-25 11:18:59 · 点赞 0

这是一个常见的 Docker 问题：当挂载的源文件不存在时，Docker 会自动创建一个目录而不是文件。项目中缺少 monitoring 目录和相关配置文件。当 Docker 挂载不存在的文件时，会自动创建成目录。

评论 #2

young.lv · 2026-01-15 20:24:38 · 点赞 0

使用docker部署，DEFAULT_MODEL=qwen-plus# MySQL数据库配置MYSQL_HOST=localhostMYSQL_PORT=3306MYSQL_USER=emotional_chatMYSQL_PASSWORD=your_secure_password1 mysql是安装在哪里的，会自动安装到docker中吗？2 向量数据库也需要手动安装吗

讨论串（1）

讨论 #1

袁从德 · 2026-01-25 11:17:45 · 点赞 0

1、MySQL 会自动安装在 Docker 中，不需要手动安装。2、向量数据库，不需要手动安装，项目使用的是 ChromaDB（嵌入式向量数据库）。ChromaDB 特点：作为 Python 包安装（已在 requirements.txt 中定义）

评论 #3

Candg · 2026-01-11 15:59:11 · 点赞 0

# 安装系统依赖RUN apt-get update && apt-get install -y \    gcc \    g++ \    cmake \    && rm -rf /var/lib/apt/lists/*Docker Compose 启动的时候，上面的命令下载文件太慢了，请问有什么建议吗

讨论串（1）

讨论 #1

袁从德 · 2026-01-12 19:54:10 · 点赞 0

可以使用国内镜像加速http://mirrors.aliyun.com/debian/

评论 #4

图论llana · 2026-01-10 21:10:20 · 点赞 0

这个项目可以在手机端打开吗，怎么打包呀，有教程吗

讨论串（1）

讨论 #1

袁从德 · 2026-01-12 20:06:56 · 点赞 0

前端是web服务，通过浏览器就可以打开没有专门的移动端打包教程（因为这是 Web 应用，不是原生 App，无需打包 APK/IPA）。

评论 #5

enjoylearning · 2026-01-05 22:58:59 · 点赞 0

目前的架构支持10万用户没问题吧？当然如果有音频文件转写，AI摘要总结就需要把耗时长的任务移到后台队列。再随着用户量的增加还要考虑微服务

讨论串（1）

讨论 #1

袁从德 · 2026-01-07 11:47:33 · 点赞 0

我们有个to G的商业化版本，不过后台是go写的，目标是支持100万级别的用户这个版本10万是可以的，主要瓶颈在与QPS增加，模型token吞吐是一个限制，如果考虑成本可以私有部署一个QWen 8B的模型，

enjoylearning 回复 袁从德 · 2026-01-07 17:12:23

那确实可以，刚开始我还想把python fastapi也搞成微服务

评论 #6

enjoylearning · 2026-01-05 00:43:37 · 点赞 1
